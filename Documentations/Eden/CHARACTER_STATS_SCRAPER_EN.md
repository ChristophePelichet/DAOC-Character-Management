# Character Statistics Scraper - Technical Documentation

## Overview

**Purpose**: Complete statistics update system for character profiles  
**Components**: CharacterProfileScraper + WealthManager  
**Category**: Profile scraping and realm-wide wealth management

---

## Table of Contents

1. [CharacterProfileScraper Class](#characterprofilescraper-class)
   - [Class Architecture](#class-architecture)
   - [Initialization](#initialization)
   - [Connection Management](#connection-management)
   - [Scraping Methods](#scraping-methods)
...
2. [WealthManager Module](#wealthmanager-module)
   - [get_realm_money()](#get_realm_money)
   - [get_first_character_per_realm()](#get_first_character_per_realm)
...
**Purpose**: Retrieve total money across all realms using CharacterProfileScraper  
**Integration**: Uses the same connection system as CharacterProfileScraper
...
│  Functions:                                             │
│    ├─ get_realm_money()                                │
│    │   └─ Main function to get money per realm         │
│    │                                                    │
│    └─ get_first_character_per_realm()                  │
│        └─ Helper to find one char per realm            │
│                                                         │
│  Uses:                                                  │
│    └─ CharacterProfileScraper.scrape_wealth_money()    │
│                                                         │
│  Returns:                                               │
│    └─ Money values for Albion, Midgard, Hibernia       │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
**Purpose**: Get money values for all three realms in a single connection
...
def get_realm_money(character_folder, cookie_manager=None, headless=False):
    ...
    Args:
        ...
        cookie_manager: CookieManager instance (optional)
...
```
┌──────────────────────────────────────────────────────────────────┐
│                  get_realm_money(character_folder)               │
└──────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌──────────────────────────────────────────────────────────────────┐
...
│ ┌──────────────────────────────────────────────────────────────┐ │
│ │ get_first_character_per_realm(character_folder)              │ │
│ │ → Returns: {                                                 │ │
│ │     'Albion': {'name': 'CharName', 'url': '...'},           │ │
│ │     'Midgard': {'name': 'CharName2', 'url': '...'},         │ │
│ │     'Hibernia': None                                        │ │
│ │   }                                                          │ │
│ └──────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────┘
...
│ ┌──────────────────────────────────────────────────────────────┐ │
│ │ For each realm (Albion, Midgard, Hibernia):                 │ │
│ │   If character exists for realm:                            │ │
...
```python
{
    'Albion': '1234g 56s 78c',      # Money string or "0"
    'Midgard': '567g 89s 12c',      # Money string or "0"
    'Hibernia': '0',                # No character found
...
result = get_realm_money('D:/Projets/Python/DAOC-Character-Management/Characters')
...
if result['success']:
    ...
    # Output:
    # Albion: 1234g 56s 78c
    # Midgard: 567g 89s 12c
    # Hibernia: 0
else:
...
1. Browse all season folders (S3, S2, S1, etc.) from newest to oldest
2. For each realm (Albion, Midgard, Hibernia):
   - Find first character with level ≥ 11 (out of tutorial)
   - Use that character's URL to scrape money
3. If no character found for a realm: Use "0" as money value

**Performance**: Single connection (~6s) + 3 wealth scrapes (~6s each) = **~24 seconds total**

---

### get_first_character_per_realm()

**Purpose**: Find one representative character per realm for money scraping
...
**Selection Criteria**:
1. **Level Requirement**: Character must be level 11+ (tutorial complete)
2. **Season Priority**: Most recent season first (S3 > S2 > S1)
3. **First Match**: Uses first valid character found per realm
...
characters = get_first_character_per_realm('Characters/')

print(characters)
# Output:
# {
#     'Albion': {'name': 'Merlin', 'url': 'https://eden-daoc.net/herald?n=player&k=Merlin'},
#     'Midgard': {'name': 'Thor', 'url': 'https://eden-daoc.net/herald?n=player&k=Thor'},
#     'Hibernia': None  # No character found
# }
```

**Why Level 11+?**
Characters below level 11 are still in the tutorial zone and don't have access to the full game economy. Their money values are not representative of account wealth.
...
2. **`search_herald_character()`** - Character search (see CHARACTER_SEARCH_SCRAPER_EN.md)
3. **`scrape_character_from_url()`** - Character update (see CHARACTER_PROFILE_SCRAPER_EN.md)
...
|---------|------|---------|
| 0.107 | 2025-11-14 | **WealthManager**: Added `get_realm_money()` documentation - Uses same centralized connection as CharacterProfileScraper |
| 0.107 | 2025-11-13 | **CharacterProfileScraper**: Refactored to use `_connect_to_eden_herald()` - Eliminated ~300 lines of duplicated connection code |
...
- **Search**: [CHARACTER_SEARCH_SCRAPER_EN.md](CHARACTER_SEARCH_SCRAPER_EN.md)
- **Update**: [CHARACTER_PROFILE_SCRAPER_EN.md](CHARACTER_PROFILE_SCRAPER_EN.md)
- **WealthManager**: Integrated in this document (see [WealthManager Module](#wealthmanager-module))
...
**Status**: ✅ Production Ready - Includes WealthManager Integration
